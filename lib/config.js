  ...
  return {
    ...
    defaults: {
      ...
      server: {
        ...
        https: false,
        httpsPort: 443,
        httpsKeyFilePath: "/path/to/https/key",
        httpsCertFilePath: "/path/to/https/certificate"
      },
    ...
    validate: function() {
      ...
      if (config.server.https){
        if (typeof config.server.httpsPort != "number"){
            error = "Invalid HTTPS port number.";
            return false;
        }
        if(config.server.httpsKeyFilePath == undefined || config.server.httpsKeyFilePath == "" || config.server.httpsKeyFilePath == "/path/to/https/key"){
            error = "Invalid or missing Key File Path for HTTPS support.";
            return false;
        }
        if(config.server.httpsCertFilePath == undefined || config.server.httpsCertFilePath == "" || config.server.httpsKeyFilePath == "/path/to/https/certificate"){
            error = "Invalid or missing Certificate File Path for HTTPS support.";
            return false;
        }
      }
