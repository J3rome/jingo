  ...
  return {
    ...
    validate: function() {
      ...
      if (config.server.https){
        ...
        if(config.server.httpsKeyFilePath == undefined || config.server.httpsKeyFilePath == ""){
            error = "Invalid or missing Key File Path for HTTPS support.";
            return false;
        }
        if(config.server.httpsCertFilePath == undefined || config.server.httpsCertFilePath == ""){
